...
public class MatrixUtils {
    ...
    public static double[][] accumulateVertical(double[][] m) {
        double [][] newArr = new double[m.length][m[0].length];
        for (int i = 0; i < m.length; i += 1) {
            for (int j = 0; j < m[0].length; j += 1) {
                ...
                if (i == 0) {
                    ...
                    newArr[i][j] = current + Math.min(newArr[i-1][j], newArr[i-1][j+1]);
                } else if (j == m[0].length - 1) {
                    newArr[i][j] = current + Math.min(newArr[i-1][j], newArr[i-1][j-1]);
                } else {
                    newArr[i][j] = current + Math.min(newArr[i-1][j+1], Math.min(newArr[i-1][j-1], newArr[i-1][j]));
                }
            }
        }
        return newArr;
