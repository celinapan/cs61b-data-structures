...
class WeirdListClient {
    ...
    static WeirdList add(WeirdList L, int n) {
        Add addFunc = new Add(n);
        return L.map(addFunc);
    }

    /** Return the sum of all the elements in L. */
    static int sum(WeirdList L) {
        Accumulator accumFunc = new Accumulator();
        L.map(accumFunc);
        return accumFunc.getAccumulator();
    }
    ...
    /** Add Class
     * Takes in n as a parameter and adds it to each element
     */
    ...
    /** Accumulator Class
     * No params but initializes an _ACCUMULATOR variable to sum up elements.
     */
    private static class Accumulator implements IntUnaryFunction {
        public Accumulator() {
            _accumulator = 0;
        }
        @Override
