...
public class NFA {
    ...
    private class State {
         ...
         * return an empty Set.
         *
         * Source of DFS: Lecture 33 & TechieDelight Iterative & Recursive Depth First
         * Links: https://www.techiedelight.com/depth-first-search/
         * */
        public Set<State> successors(char c) {
            // TODO: Implement this method
            if (!this._edges.containsKey(c)) {
                return new HashSet<>();
            }
            if(c == EPSILON) {
                Set<State> fringes = new HashSet<>(this._edges.get(c));
                Set<State> res = new HashSet<>();
                for (State st: fringes) {
                    res.addAll(st.successors(c));
                }
                fringes.addAll(res);
                return fringes;
            } else {
     ...
     * Source: Pseudo-code provided on HW9 Specs*/
    public boolean matches(String s) {
        // TODO: write the matching algorithm
        Set<State> current = new HashSet<>();
        current.add(_startState);
        current.addAll(_startState.successors(EPSILON));
        for (char c: s.toCharArray()) {
            HashSet<State> newSet = new HashSet<>();
            for(State st: current) {
                newSet.addAll(st.successors(c));
            }
            current = newSet;
            newSet = new HashSet<>();
            for (State st: current) {
                newSet.addAll(st.successors(EPSILON));
            }

            current.addAll(newSet);
        }

        return current.contains(_acceptState);
    }
