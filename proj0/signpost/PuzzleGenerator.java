...
class PuzzleGenerator implements PuzzleSource {
    ...
    static Sq findUniqueSuccessor(Model model, Sq start) {
        ...
        if (startSuccessors == null) {
            return null;
        }
        if (start.sequenceNum() != 0) {
            for (Place pl: startSuccessors) {
                currentSq = model.get(pl);
                if (currentSq.sequenceNum() == start.sequenceNum() + 1) {
                    return currentSq;
                } else if (currentSq.sequenceNum() == 0) {
                    uniqueSq = currentSq;
                    connectable += 1;
                }
            }
    ...
    static Sq findUniquePredecessor(Model model, Sq end) {
        PlaceList endPredeccessors = end.predecessors();
        int connectable = 0;
        Sq uniqueSq = null;
        Sq currSq;

        if (endPredeccessors == null) {
            return null;
        }
        for (Place pl: endPredeccessors) {
            currSq = model.get(pl);
            if (currSq.connectable(end)) {
                uniqueSq = currSq;
                connectable += 1;
            }
