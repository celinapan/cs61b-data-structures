...
class Model implements Iterable<Model.Sq> {
    ...
    Model(int[][] solution) {
        ...
        if (!present) {
            throw new IllegalArgumentException(
                    "Numbers 1 - last does not all appear.");
        }
        for (Sq sq: _allSquares) {
            sq._successors = Place.successorCells(
                                width(), height())[sq.x][sq.y][sq.direction()];
            for (Place succ: sq._successors) {
                get(succ)._predecessors.add(sq.pl);
            }
        }
        _unconnected = last - 1;
    }
    ...
    Model(Model model) {
        ...
        _board = new Sq[_width][_height];
        for (Sq sq: model._allSquares) {
            _board[sq.x][sq.y] = new Sq(model.get(sq.x, sq.y));
            _allSquares.add(_board[sq.x][sq.y]);
        }
        for (Sq sq: _allSquares) {
            Sq oldSq = model.get(sq.x, sq.y);
            sq._successor = oldSq.successor();
            sq._predecessor = oldSq.predecessor();
        }
    }
    ...
    boolean autoconnect() {
        boolean fl = false;
        for (Sq sq :_allSquares) {
            if (sq.sequenceNum() != 0 && sq._successor == null) {
                for (Place p : sq.successors()) {
                    Sq curr = get(p);
                    if (curr.sequenceNum() == sq.sequenceNum() + 1) {
                        fl = sq.connect(curr);
                    }
                }
            }
    ...
    final class Sq {
        ...
        boolean connect(Sq s1) {
            ...
            if (this.sequenceNum() > 0) {
                for (Sq s = this; s._successor != null; s = s._successor) {
                    s._successor._sequenceNum = s._sequenceNum + 1;
                }
                releaseGroup(sgroup);
            }
            if (s1.sequenceNum() > 0) {
                int currGrp = this.group();
                for (Sq s = s1; s._predecessor != null; s = s._predecessor) {
                    s._predecessor._sequenceNum = s._sequenceNum - 1;
                }
                releaseGroup(currGrp);
            }
    ...
    /** main() tests the arrowDirection method should checks
     * to see the the return value is as expected.
     * @param args */
    public static void main(String[] args) {
        ...
        Model m = new Model(soln1);
        System.out.println("Should return 3 (1,1) --> (2,0): "
                + m.arrowDirection(1, 1));
    }
}
